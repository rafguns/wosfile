...
addresses = [
    ...
    (
        (
            ...
            "Univ Leuven, Dept Earth & Environm Sci, Leuven, Belgium; "
            "[Bi, Lingling; Vanneste, Dominique] Univ Leuven, Dept Earth & Environm Sci, Leuven, Belgium; "  # noqa: E501
...
@pytest.mark.parametrize(("field", "expected", "exception"), addresses)
def test_parse_address_field(field, expected, exception):
    """Correctly parse C1 (address) fields"""
    if exception is not None:
        with pytest.raises(exception):
            parse_address_field(field)
    else:
        res = parse_address_field(field)
        assert res == expected
